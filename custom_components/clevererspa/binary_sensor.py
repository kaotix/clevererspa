...
async def async_setup_entry(
    ...
    entities: list[CleverSpaEntity] = []
    for device_id in coordinator.data.keys():
        entities.extend(
            [
                CleverSpaConnectivitySensor(coordinator, config_entry, device_id),
                CleverSpaErrorSensor(coordinator, config_entry, device_id),
            ]
        )
...
class CleverSpaConnectivitySensor(CleverSpaBinarySensor):
...
class CleverSpaErrorSensor(CleverSpaBinarySensor):
    ...
    def is_on(self) -> bool | None:
        ...
        return len(self.device_status.errors) > 0
